    ...
    <el-row class="ocr-result">
      ...
      <div class="result-details">
        <el-table :data="tableData" height="400" style="width: 100%" v-loading="isRequesting">
          <el-table-column :label="$t('confidence')" align="left">
            <template slot-scope="scope">
              <el-progress
                :text-inside="true"
                :stroke-width="30"
                :percentage="parseInt(scope.row.conf*100)"
                :status="scope.row.type==1?'success':'exception'"
              ></el-progress>
            </template>
          </el-table-column>
          <el-table-column :label="$t('recog_result')" align="left">
            <template slot-scope="scope">{{scope.row.type==1?$t('hasword'):$t('noword')}}</template>
          </el-table-column>
        </el-table>
      </div>
    </el-row>
...
<script>
const imgArrOrigin = [
  { url: "/static/images/worddetect/worddetect_1.jpg" },
  { url: "/static/images/worddetect/worddetect_2.jpg" },
  { url: "/static/images/worddetect/worddetect_3.jpg" },
  { url: "/static/images/worddetect/worddetect_4.jpg" },
  { url: "/static/images/worddetect/worddetect_5.jpg" },
  { url: "/static/images/worddetect/worddetect_6.jpg" }
];
...
export default {
  ...
  methods: {
    init(url) {
      ...
      if (this.input_url != "") {
        this.tengxunOcr({ url: this.input_url }, this.imgOptions);
      } else {
        this.getImageToBase64Data(this.imageUrl).then(params => {
          this.tengxunOcr(params, this.imgOptions);
        });
      }
    },

    //接口请求方法封装
    tengxunOcr(params, options = {}) {
      ...
      api.tengxunApi
        ...
        .then(res => {
          ...
          if (res.status == 200) {
            ...
            if (errorcode === 0) {
              this.tableData.push(resData.word_detect);
            }
          }
        })
        .catch(error => {
